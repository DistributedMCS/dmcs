...
/**
 * @file   ContextGeneratorBase.cpp
 * @author Minh Dao-Tran <dao@kr.tuwien.ac.at>
 * @date   Wed Apr  30 04:39:20 2014
 * 
 * @brief  
 * 
 * 
...
addUniqueBridgeAtom(BridgeRulePtr& r, std::size_t neighbor_id, int neighbor_atom)
{
  ...
  DBGLOG(DBG, "Adding atom " << neighbor_atom << " of neighbor " << neighbor_id);
...
void
ContextGeneratorBase::generate_bridge_rule(std::size_t id)
{
...
void
ContextGeneratorBase::generate_bridge_rule_list(std::size_t id)
{
  do
    {
      bridge_rules->clear();
...
bool
ContextGeneratorBase::cover_neighbors(std::size_t id)
{
  ...
  for (; i != bridge_rules->end(); ++i)
      ...
      for (PositiveBridgeBody::const_iterator j = pb.begin(); j != pb.end(); ++j)
	{
	  ...
	  DBGLOG(DBG, "insert neighbor id" << j->first);
	}
  ...
  DBGLOG(DBG, "neighbor size: " << nbors.size());
  DBGLOG(DBG, "orig nb " << id << ", size: " << ( (*orig_topo)[id]->size() ) );
...
void
ContextGeneratorBase::write_local_kb(std::size_t id)
{
...
void
ContextGeneratorBase::write_bridge_rules(std::size_t id)
{
  std::stringstream out;
  out << id;
...
NewBeliefStatePtr
ContextGeneratorBase::local_interface(/* std::size_t id1, */ std::size_t id2)
{
...
void
ContextGeneratorBase::update_min_V()
{
