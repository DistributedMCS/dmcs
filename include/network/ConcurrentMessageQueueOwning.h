...
namespace dmcs {
  ...
  private:
    ...
    struct Block
    {
      // pointer to the message, shares ownership of message
      MessagePtr m;
      // TODO add metadata (priority and so on)
    };
    ...
    // TODO this should be called pushMessage and take message metadata as arguments, or it should take a const Block& as argument
    inline void
    pushBlock (MessagePtr m)
    {
      assert(m);
      Block b = { m };
      q.push(b);
    ...
    // TODO this should be called popMessage (including metadata)
    inline void 
    popBlock (MessagePtr& m)
    {
      Block& b = q.front();
      assert(!!b.m);
      // save b.m
      m = b.m;
      // this destructs b.m (so if we don't assign it above it is lost)
      q.pop();
    }
