...
namespace dmcs {
   ...
   */
  template<typename MessageBaseClass>
  ...
  private:
    ...
    struct Block
    {
      MessageBaseClass *m; // the message
    };

    std::queue<Block> q;   /// holds data of message queue
    const std::size_t n;   /// capacity of message queue (elements, not bytes)
    ...
    inline void
    pushBlock (const MessageBaseClass *msg)
    {
      Block b = { msg };
      q.push(b);
    }
  ...
  public:
    ...
    void
    send (const MessageBaseClass* buf, unsigned int /* prio */)
    {
      boost::mutex::scoped_lock lock(mtx);
      waitOnCapacity(lock);
      pushBlock(buf);
    ...
    bool
    try_send (const MessageBaseClass* buf, unsigned int /* prio */)
    {
      boost::mutex::scoped_lock lock(mtx);
    ...
    bool
    timed_send (const MessageBaseClass* buf, unsigned int /* prio */, const boost::posix_time::time_duration& t)
    {
      boost::mutex::scoped_lock lock(mtx);
